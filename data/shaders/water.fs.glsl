...
uniform float particle_x[20];
uniform float particle_y[20];
in vec2 texcoord;
...
vec4 mainImage( vec2 uv )
{
	...
	{
		if(particle_x[sampleIndex] <= 0) break;
		#ifdef motion_blur
		...
		vec2 p = vec2(particle_x[sampleIndex]/1600.0*14.0-0.5, particle_y[sampleIndex]/800.0*15.0);
		//		vec2 p = vec2(-13.0,1.0);
